...
<template>
  <div class="inner draw" @mousemove="beginPath($event)">
    <div class="wrap">
      ...
      <!-- <img id="scream" src="https://dss3.bdstatic.com/70cFv8Sh_Q1YnxGkpoWK1HF6hhy/it/u=1208538952,1443328523&fm=26&gp=0.jpg" alt=""> -->
      <div id="control" class="fl">
        ...
        <div id="canvas-control">
          <h5>操作</h5>
          <span 
            v-for="(control,keys) in controls" 
            :title="control.title" 
    ...
    <div id="img-box" v-show="imgUrl.length">
      <div class="img-item" v-for="(src,key) in imgUrl" :key="key">
        <img :src="src">
        <span class="fa el-icon-circle-close" @click="removeImg(src)"></span>
      </div>
    </div>
  </div>
</template>
  ...
  export default {
    data () {
      return {
        ...
        brushs: [{
          className: 'small fa el-icon-edit',
          lineWidth: 3
        }, {
          className: 'middle fa el-icon-edit',
          lineWidth: 6
        }, {
          className: 'big fa el-icon-edit',
          lineWidth: 12
        }],
        context: {},
        imgUrl: [],
    ...
    mounted () {
      ...
      img.src = this.url
      this.context.drawImage(img,20,10);
      this.initDraw()
      this.setCanvasStyle()
    },
    watch:{
    ...
    computed: {
      controls () {
        return [{
          title: '上一步',
          action: 'prev',
          className: this.preDrawAry.length ? 'active fa el-icon-back' : 'fa el-icon-back'
        }, {
          title: '下一步',
          action: 'next',
          className: this.nextDrawAry.length ? 'active fa el-icon-right' : 'fa el-icon-right'
        }, {
          title: '清除',
          action: 'clear',
          className: (this.preDrawAry.length || this.nextDrawAry.length) ? 'active fa el-icon-delete' : 'fa el-icon-delete'
        }]
      }
    },
    methods: {
